...
function RecoverEmailContent() {
  ...
  useEffect(() => {
    ...
    const deepLink = `dreamweaver://auth?${params.toString()}`;
    ...
    return (
      <div className="min-h-screen bg-navy-deep flex items-start justify-center px-4 pt-16 pb-8">
        <div className="text-center text-foreground max-w-md">
          ...
          <h1 className="text-2xl font-brand text-primary golden-text-glow mb-4">Opening DreamWeaver App...</h1>
          <p className="text-text-secondary">
            We&apos;re redirecting you to the DreamWeaver app to recover your email.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-navy-deep flex items-start justify-center px-4 pt-16 pb-8">
      <div className="bg-navy-light/80 backdrop-blur-md rounded-lg p-6 max-w-md w-full text-center text-foreground border border-border">
        ...
        <h1 className="text-xl font-brand text-primary golden-text-glow mb-4">Recover email</h1>
        
        {oobCode ? (
          <div>
            <p className="mb-6 text-text-secondary">
              We couldn&apos;t automatically open the DreamWeaver app. Please open the app manually to recover your email.
            </p>
            
            <div className="bg-black/20 rounded p-4 mb-6">
              <p className="text-sm text-gray-300 mb-2">Or copy this recovery code:</p>
              <code className="text-yellow-300 break-all text-sm">{oobCode}</code>
            </div>
            
            <button 
              onClick={() => window.location.href = 'dreamweaver://recover'}
            ...
            >
              Open DreamWeaver App
            </button>
          </div>
        ) : (
          <p className="text-error">Invalid recovery link. Please try again or contact support.</p>
        )}
